(* yoshika takezawa  -- TypeDecl, Variant, Case
...
and
  ...
  | Variant(tag,args) -> 
    TaggedVariantVal(tag, (List.map (fun x -> eval_expr en x) args))
  | Case(cond,branches) ->
    let c = eval_expr en cond
    in let rec find_branch= function
        | [] -> failwith "Interp: Case: could not find branch"
        | x::xs -> 
          let Branch(tag, args, output) = x
          in (match c with 
              | TaggedVariantVal(t, a) when t = tag -> 
                  eval_expr (extend_env_list en args a) output
              | _ -> find_branch xs)
