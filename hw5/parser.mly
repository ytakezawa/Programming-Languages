...
%right ARROW
%nonassoc REFTYPE                          (*%nonassoc UMINUS        /* highest precedence */*)
...
expr:
    ...
    | id = CONSTRUCTOR; LANGLE; es = separated_list(COMMA, expr);
      RANGLE {Variant(id,es) }
    | CASE; body=expr; OF; LBRACE;
      bs = nonempty_list(branch); RBRACE { Case(body,bs) }
    | TYPE; id=ID; EQUALS; cs = nonempty_list(cdecl)
                                  { TypeDecl(id,cs) }
...
branch:
    | id = CONSTRUCTOR; ARROW; tgt=expr {Branch(id,[],tgt) }
      ;
    | id = CONSTRUCTOR; LANGLE; es = separated_list(COMMA, ID);
      RANGLE; ARROW; tgt=expr {Branch(id,es,tgt) }
      ;
cdecl:
    | PIPE; id = CONSTRUCTOR {CDec(id,[]) }
    | PIPE; id = CONSTRUCTOR; LANGLE; es = separated_list(COMMA, texpr);
      RANGLE;  {CDec(id,es) }
      ;
